const String linkserverName = "https://techshowvip.000webhostapp.com/notesapp";

//auth
const String signuplink =
    "https://techshowvip.000webhostapp.com/notesapp/auth/signup.php";
const String loginlink =
    "https://techshowvip.000webhostapp.com/notesapp/auth/login.php";
const String addlink =
    "https://techshowvip.000webhostapp.com/notesapp/notes/add.php";
const String addwitoutphoto =
    "https://techshowvip.000webhostapp.com/notesapp/notes/addwithout.php";
const String viewslink =
    "https://techshowvip.000webhostapp.com/notesapp/notes/views.php";
const String viewsbooklink =
    "https://techshowvip.000webhostapp.com/notesapp/notes/views_book.php";

const String deletelink = "https://10.0.2.2/notesapp/notes/delete.php";
const String editlink =
    "https://techshowvip.000webhostapp.com/notesapp/notes/edit.php";
const String imageroot = "http://techshowvip.000webhostapp.com/notesapp/upload";
const String addbooklink =
    "https://techshowvip.000webhostapp.com/notesapp/notes/add_book.php";
const String addprofilelink =
    "https://techshowvip.000webhostapp.com/notesapp/notes/add_profile.php";

//local host
// String linkserverName = "http://127.0.0.1/notesapp";

//auth
// const String Signuplink = "http://10.0.2.2/notesapp/auth/signup.php";
// const String Loginlink = "http://10.0.2.2/notesapp/auth/login.php";
// const String addlink = "http://10.0.2.2/notesapp/notes/add.php";
// const String addwitoutphoto = "http://10.0.2.2/notesapp/notes/addwithout.php";
// const String viewslink = "http://10.0.2.2/notesapp/notes/views.php";
// const String viewsbooklink = "http://10.0.2.2/notesapp/notes/views_book.php";

// const String deletelink = "http://10.0.2.2/notesapp/notes/delete.php";
// const String editlink = "http://10.0.2.2/notesapp/notes/edit.php";
// const String imageroot = "http://10.0.2.2/notesapp/upload";
// const String addbooklink = "http://10.0.2.2/notesapp/notes/add_book.php";
// const String addprofilelink = "http://10.0.2.2/notesapp/notes/add_profile.php";
